...
export class departmentController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/department/:id', this.GpDelete);
this.router.get('/department/get/search', this.GpSearch);
this.router.put('/department', this.GpUpdate);
this.router.put('/department/get/update', this.GpSearchForUpdate);
this.router.get('/department', this.GpGetAllValues);
this.router.post('/department', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into departmentController.ts: GpDelete');
        new ApiAdaptar().delete(Constant.DEPARTMENTURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdaptar().get(Constant.DEPARTMENTURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpUpdate(req: Request, res: Response) {
        ...
        new ApiAdaptar().put(Constant.DEPARTMENTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdaptar().post(Constant.DEPARTMENTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
